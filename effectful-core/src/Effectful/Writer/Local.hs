...
runWriter m = do
  (a, IdA (Writer w)) <- runEffect (IdA (Writer mempty)) m
  pure (a, w)
...
listen m = unsafeEff $ \es -> mask $ \restore -> do
  ...
  where
    merge es w0 =
      ...
      stateEnv es $ \(IdA (Writer w1)) -> (w1, IdA (Writer (w0 <> w1)))
