...
-- | Convenience functions for the 'Labeled' 'Reader' effect with visible
-- dependent quantification.
--
-- Requires GHC >= 9.10.
--
-- @since 2.4.0.0
module Effectful.Labeled.Reader.VDQ
  ( -- * Effect
    Reader(..)
...
runReader
  ...
  -> Eff es a
runReader label = runLabeled label . R.runReader
...
ask
  ...
  => Eff es r
ask label = send $ Labeled @label Ask

-- | Retrieve a function of the current environment.
--
-- @'asks' label f ≡ f '<$>' 'ask' label@
asks
  :: forall label
  -> (HasCallStack, Labeled label (Reader r) :> es)
  => (r -> a)
...
local
  ...
  -> Eff es a
  -> Eff es a
local label f = send . Labeled @label . Local f
